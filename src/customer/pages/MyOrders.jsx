 ...
 const findCustomerRecord = async () => {
  try {
    ...
    const res = await axios.get(
      `/customers?page=1&pageSize=100&search=${customer.email}`
    );
    ...
    if (!found) {
      const res2 = await axios.get(
        `/customers?page=1&pageSize=100&search=${customer.fullName}`
      );
      found = res2.data.data?.[0];
    }

    // ✅ If still not found create customer record automatically
    if (!found) {
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      if (!customerRecord) {
        alert('❌ Customer profile not found. Please contact store.');
        setPlacing(false);
        return;
      }
...
<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
  <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
    ...
    <div style={{
      ...
      boxShadow: '0 4px 16px rgba(224,68,114,0.3)',
    }}>
    ...
    <div>
      ...
      <p style={{ color: '#6b7280', fontSize: '14px' }}>
        Welcome back, <span style={{ color: '#e04472', fontWeight: '600' }}>{customer?.fullName}</span>!
      </p>
    </div>
  </div>
  {cart.length > 0 && (
    <button onClick={() => setShowCheckout(true)} style={{
      ...
      🛒 Place Order ({cart.length} items • ₹{totalPrice.toLocaleString()})
    </button>
  )}
</div>
        ...
        {!loading && !customerRecord && (
          <div style={{
            ...
            <p style={{ color: '#fbbf24', fontSize: '15px' }}>
              ...
              Please contact the store to place orders.
            </p>
          </div>
        )}
            ...
            <form onSubmit={handlePlaceOrder}>
              ...
              <div style={{ marginBottom: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  {PAYMENT_METHODS.map(method => (
                    ...
                    >
                      <div style={{ fontSize: '28px', marginBottom: '6px' }}>{method.icon}</div>
                      <p style={{
                      ...
                      <p style={{ color: '#6b7280', fontSize: '11px' }}>{method.desc}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
                <button type="submit" disabled={placing} style={{
                  ...
                  cursor: placing ? 'not-allowed' : 'pointer', fontSize: '15px',
                }}>
                  {placing ? 'Placing Order...' : `✅ Pay ₹${totalPrice.toLocaleString()} via ${checkoutForm.paymentMethod}`}
                </button>
